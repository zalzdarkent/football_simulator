  ...
  head: () => ({
    meta: [
      { title: "Become a Legend — Mulai Karier Sepakbolamu" },
      {
        name: "description",
        content:
          "Buat pemain, spin pertandingan, kumpulkan trofi. Simulator karier sepakbola yang bikin ketagihan.",
      },
    ],
  }),
  component: Landing,
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {active.map((s) => (
              <SaveCard
                ...
                save={s}
                onOpen={() => {
                  void setActive(s.id).then(() => navigate({ to: "/dashboard" }));
                }}
                onDelete={() => {
            ...
            {retired.map((s) => (
              <SaveCard
                ...
                retired
                onOpen={() => {
                  void setActive(s.id).then(() => navigate({ to: "/dashboard/journey" }));
                }}
                onDelete={() => {
                  void deleteSave(s.id);
                }}
...
function MockCard({
  ...
  return (
    <div
      className={`w-56 aspect-[3/4] rounded-2xl ${gold ? "bg-gold-gradient" : "bg-pitch-gradient"} text-primary-foreground shadow-2xl relative overflow-hidden`}
    >
      <div className="absolute top-3 left-3">
        ...
        <div
          className="mt-2 w-8 h-8 rounded-md border border-white/30 flex items-center justify-center text-[9px] font-bold"
      ...
      <div className="absolute inset-x-0 top-[45%] flex justify-center">
        <div className="w-24 h-24 rounded-full bg-white/10 flex items-center justify-center text-4xl">
          👤
        </div>
      </div>
      <div className="absolute inset-x-0 bottom-6 text-center font-display font-bold text-lg">
        {name}
      </div>
    </div>
  );
...
function SaveCard({
  ...
  save: ReturnType<typeof useStore.getState>["saves"][number];
  onOpen: () => void;
  onDelete: () => void;
  retired?: boolean;
}) {
  ...
  return (
    <Card className="bg-card-gradient border-border/60 overflow-hidden hover:border-primary/30 transition-colors">
      <CardContent className="p-3">
        <div className="flex gap-3 items-start">
          <div className="shrink-0">
            <PlayerCard save={save} size="xs" />
          </div>
          <div className="min-w-0 flex-1 pt-1">
            <div className="flex items-center gap-2">
              <div className="font-display font-bold truncate text-sm">{save.player.name}</div>
              {retired && (
                <span className="text-[10px] uppercase bg-muted rounded px-1.5 py-0.5 shrink-0">
                  Pensiun
                </span>
              )}
            </div>
            <div className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
              ...
              {save.player.position} • {club?.short ?? "—"} • Umur {save.player.age}
            </div>
            ...
            <div className="mt-2 flex gap-1.5">
              ...
              <Button
                ...
                variant="ghost"
                onClick={() => {
                  if (confirm("Hapus karier ini?")) onDelete();
                }}
