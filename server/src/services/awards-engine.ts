...
function buildNominees(
  ...
  for (let i = 0; i < count * 2; i++) {
    ...
    rivals.push({ name: fakePlayer(rng), club_id: club.id, score: Math.max(0, opts.rivalBaseline + noise) });
  }
  const all = [
...
export async function computeSeasonAwards(agg: PlayerSeasonAgg, seed: number): Promise<AwardResult[]> {
  ...
  if (!isGK) {
    ...
    results.push(mk("golden-boot", buildNominees(agg, rngFor(seed, "gb", agg.season_idx), 5, score, { requireLeague: true, clubs, rivalBaseline: gbBaseline, rivalSpread: 40 }), `${agg.goals} gol`));
    ...
    results.push(mk("top-assist", buildNominees(agg, rngFor(seed, "ta", agg.season_idx), 5, aScore, { requireLeague: true, clubs, rivalBaseline: taBaseline, rivalSpread: 30 }), `${agg.assists} assist`));
  }
  ...
  if (isGK) {
    ...
    results.push(mk("golden-glove", buildNominees(agg, rngFor(seed, "gg", agg.season_idx), 5, gScore, { requireLeague: true, clubs, rivalBaseline: ggBaseline, rivalSpread: 40 }), `${agg.clean_sheets} clean sheet`));
  }
  ...
  if (agg.age <= 21) {
    ...
    results.push(mk("best-young", buildNominees(agg, rngFor(seed, "by", agg.season_idx), 5, yScore, { requireLeague: true, clubs, rivalBaseline: byBaseline, rivalSpread: 28 }), `Rating rata-rata ${agg.avg_rating}`));
  }
  // POTY Liga — baseline: setara musim bagus di liga kelas atas (rating 8.0, 18 gol, 8 assist, 10 motm, 38 apps)
  {
    ...
    results.push(mk("poty-league", buildNominees(agg, rngFor(seed, "poty", agg.season_idx), 6, perfScore, { requireLeague: true, clubs, rivalBaseline: potyBaseline, rivalSpread: 40 })));
  }
  // TOTS (11) — kamu masuk atau tidak; threshold rating dinaikkan jadi 7.8
  {
    const rng = rngFor(seed, "tots", agg.season_idx);
    const inTOTS = agg.avg_rating >= 7.8 && agg.apps >= 20 && chance(0.50, rng);
  ...
  if (agg.trophies.includes("ucl") || agg.avg_rating >= 7.8) {
    ...
    results.push(mk("ucl-poty", buildNominees(agg, rngFor(seed, "uclp", agg.season_idx), 5, uclScore, { clubs, rivalBaseline: uclBaseline, rivalSpread: 50 })));
    // UCL Top Scorer — baseline: setara ~8 gol UCL
    if (!isGK) results.push(mk("ucl-top-scorer", buildNominees(agg, rngFor(seed, "uclt", agg.season_idx), 5, agg.goals * 8, { clubs, rivalBaseline: 8 * 8, rivalSpread: 25 }), `${Math.max(agg.goals, 0)} gol UCL`));
  }
  ...
  results.push(mk("ballon-dor", buildNominees(agg, rngFor(seed, "bdor", agg.season_idx), 10, worldScore, { clubs, rivalBaseline: bdorBaseline, rivalSpread: 65 })));

  // FIFA The Best — sedikit lebih mudah dari Ballon d'Or tapi tetap ketat
  const fifaBaseline = 245;
  results.push(mk("fifa-best", buildNominees(agg, rngFor(seed, "fifab", agg.season_idx), 7, worldScore * 0.95 + range(-5, 5, rngFor(seed, "fifabn", agg.season_idx)), { clubs, rivalBaseline: fifaBaseline, rivalSpread: 60 })));

  // UEFA POTY — baseline agak lebih rendah, fokus di Eropa
  const uefaBaseline = 220;
  results.push(mk("uefa-poty", buildNominees(agg, rngFor(seed, "uefap", agg.season_idx), 5, worldScore + (agg.ucl_won ? 20 : 0), { clubs, rivalBaseline: uefaBaseline, rivalSpread: 55 })));

  // Golden Boy (dunia, u21) — baseline lebih rendah tapi tetap butuh musim bagus
  if (agg.age <= 21) {
    const gboyBaseline = 160;
    results.push(mk("golden-boy", buildNominees(agg, rngFor(seed, "gboy", agg.season_idx), 10, worldScore * 1.2, { clubs, rivalBaseline: gboyBaseline, rivalSpread: 50 })));
  }
